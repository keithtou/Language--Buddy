...
function StudentCard(props) {
    ...
    let languageName = languageList.find(el => el.id === props.students["language"]).name;
    let languageLevel = levels.find(el => el.id === props.students["language_level"]).name;
  ...
  return (
    <Card className="student_card" id={props.students["id"]}>
      ...
      <Card.Body className="card_body">
        ...
        <Card.Subtitle className="mb-3 text-muted">{languageName} - {languageLevel}</Card.Subtitle>
